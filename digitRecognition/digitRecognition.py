...
from calculateBatch import calculateBatch

loss = float('inf')         #starting loss
iterations = 100000            #amount of iterations, how much times it would run the optimisation
change = 0.001               #amount by which weights and biases should change each iteration
...
for i in range(0, iterations):          #cycle which will optimize NN the required number of times
    batchX, batchY = calculateBatch(X, y, batchSize, i)  #calculating batch of data for this iteration
    print(f"Iteration {i}")
    loss = iteration(batchX, batchY, denses, activations)
